...
const getFirebaseConfig = () => {
  ...
  console.log('🔍 Raw Firebase config from environment:', {
    ...
    rawApiKey: rawConfig.apiKey,
    rawAuthDomain: rawConfig.authDomain,
    rawProjectId: rawConfig.projectId,
    envVars: {
...
export const initializeFirebase = () => {
  ...
  if (!config) {
    ...
    // Create a more robust mock auth that won't cause runtime errors
    firebaseApp = {
      name: '[DEFAULT]',
      options: {},
  ...
  try {
    ...
    try {
      ...
      console.error('❌ Failed to import firebase/app:', importError);
    ...
    try {
      ...
      console.error('❌ Failed to import firebase/auth:', importError);
    ...
    console.error('4. Firebase modules not properly installed');
